...
#         self.classifier2 = nn.Linear(320 + bert_feature_dim, 2)  # 假设是二分类任务
#
#     def forward(self, data):
#         x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
#         bert_features = data.bert_features  # 确保bert_features是二维的
#
...
class GraphTransformer(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, num_heads, edge_dim, bert_feature_dim, num_classes, dropout=0.0, layer_norm=True, batch_norm=True):
        super(GraphTransformer, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(GraphTransformerLayer(in_channels, hidden_channels, edge_dim, num_heads, dropout, layer_norm, batch_norm))
        ...
        self.pool = global_mean_pool
        self.classifier1 = nn.Linear(out_channels, 320)
        self.classifier2 = nn.Linear(320 + bert_feature_dim, num_classes)  # num_classes should match the number of labels

    def forward(self, data):
        x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        bert_features = data.bert_features  # Ensure bert_features is 2D

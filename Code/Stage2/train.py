...
num_epochs = 100
best_test_absolute_true = 0.0
best_model_state = None

for epoch in range(num_epochs):
    train_loss, train_f1, train_preds, train_labels = train(model, train_loader, optimizer, clf_loss_fn,
                                                            contrastive_loss_fn)
    ...
    print(
        f'Epoch {epoch + 1}, Train Loss: {train_loss:.4f}, Train F1: {train_f1:.4f}, Test Loss: {test_loss:.4f}, Test F1: {test_f1:.4f}')
    print("Evaluation Results:")
    for metric, value in evaluation_results.items():
        print(f"{metric}: {value:.4f}")


    if evaluation_results['absolute_true'] > best_test_absolute_true:
        best_test_absolute_true = evaluation_results['absolute_true']
        best_model_state = model.state_dict()
        torch.save(best_model_state, 'best_model_with_absolute_true2.pth')

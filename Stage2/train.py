...
from loss_function import BCEFocalLoss, MultiLabelSupervisedContrastiveLoss
...
def parse_args():
    ...
    parser.add_argument("--label-contrastive-temperature", type=float, default=0.1)
    parser.add_argument("--label-contrastive-weight", type=float, default=0.5)
...
def run_epoch(
    ...
    clf_loss_fn,
    contrastive_loss_fn,
    label_contrastive_loss_fn,
    device,
    contrastive_weight,
    label_contrastive_weight,
    training,
):
    if training:
        model.train()
    ...
    total_loss = 0.0
    total_clf_loss = 0.0
    total_seq_struct_loss = 0.0
    total_label_contrastive_loss = 0.0
    all_preds = []
    all_labels = []
    ...
    for data in loader:
        ...
        with torch.set_grad_enabled(training):
            ...
            seq_struct_loss = contrastive_loss_fn(structure_proj, sequence_proj)
            label_contrastive_loss = label_contrastive_loss_fn(fused_proj, data.y)
            loss = (
                clf_loss
                + contrastive_weight * seq_struct_loss
                + label_contrastive_weight * label_contrastive_loss
            )
        ...
        total_seq_struct_loss += seq_struct_loss.item()
        total_label_contrastive_loss += label_contrastive_loss.item()
        all_preds.append(torch.sigmoid(logits).detach().cpu().numpy())
        all_labels.append(data.y.detach().cpu().numpy())

    num_batches = max(len(loader), 1)
    average_loss = total_loss / num_batches
    average_clf_loss = total_clf_loss / num_batches
    average_seq_struct_loss = total_seq_struct_loss / num_batches
    average_label_contrastive_loss = total_label_contrastive_loss / num_batches
    ...
    return (
        average_loss,
        average_clf_loss,
        average_seq_struct_loss,
        average_label_contrastive_loss,
        macro_f1,
        all_preds,
        all_labels,
    )
...
def main():
    ...
    label_contrastive_loss_fn = MultiLabelSupervisedContrastiveLoss(
        temperature=args.label_contrastive_temperature
    )

    best_absolute_true = 0.0
    for epoch in range(1, args.epochs + 1):
        train_loss, train_clf_loss, train_seq_struct_loss, train_label_contrastive_loss, train_f1, _, _ = run_epoch(
            model,
            train_loader,
            optimizer,
            clf_loss_fn,
            ...
            args.label_contrastive_weight,
            training=True,
        )
        test_loss, test_clf_loss, test_seq_struct_loss, test_label_contrastive_loss, test_f1, test_preds, test_labels = run_epoch(
            model,
            test_loader,
            optimizer,
            clf_loss_fn,
        ...
        print(
            ...
            f"TrainLoss: {train_loss:.4f} | TrainCls: {train_clf_loss:.4f} | "
            f"TrainSSCL: {train_seq_struct_loss:.4f} | TrainLSCL: {train_label_contrastive_loss:.4f} | "
            f"TrainF1: {train_f1:.4f} | "
            f"TestLoss: {test_loss:.4f} | TestCls: {test_clf_loss:.4f} | "
            f"TestSSCL: {test_seq_struct_loss:.4f} | TestLSCL: {test_label_contrastive_loss:.4f} | "
            f"TestF1: {test_f1:.4f}"
        )
